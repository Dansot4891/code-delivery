    ...
    return Column(
      children: [
        ...
        Padding(
          ...
          child: _Footer(total: (basket.firstWhere((e) => e.product.id == id).count * basket.firstWhere((e) => e.product.id == id).product.price).toString(), count: basket.firstWhere((e) => e.product.id == id).count, onSubtract: onSubtract!, onAdd: onAdd!),
        )
      ],
    );
  }
...
class _Footer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        Expanded(child: Text("총액 ₩$total", style: TextStyle(color: PRIMARY_COLOR, fontWeight: FontWeight.w500,),)),
        Row(
          children: [
            renderButton(icon: Icons.remove, onTap: onSubtract),
            SizedBox(width: 8,),
            Text(count.toString(), style: TextStyle(color: PRIMARY_COLOR, fontWeight: FontWeight.w500,),),
            SizedBox(width: 8,),
